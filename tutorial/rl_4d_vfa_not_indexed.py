...
def select_random_decision(state, initial_state, x_hl_k, x_sl_k, x_su_k,
    ...
    post_decision_state = np.round(post_decision_state, \
                                   decimals = -1)
...
def select_best_decision(state, initial_state, x_hl_k, x_sl_k, x_su_k,
    ...
    post_decision_state = np.round(current_state, decimals = -1)
    ...
    if (state['e_k'] == 1):
        ...
        post_decision_state = [np.round(current_state - \
            x_hl_k[i], decimals = -1) for i in valid_idx]

        keys = [str(x) for x in post_decision_state]
        for i, key in enumerate(keys):
            if key in v_bar_n_minus_one:
                v_bar[i] = contribution[i] + v_bar_n_minus_one[key]
            else:
    ...
    if (state['e_k'] == 2):
        ...
        post_decision_state = [np.round(current_state - \
            x_sl_k[i], decimals = -1) for i in valid_idx]

        keys = [str(x) for x in post_decision_state]

        for i, key in enumerate(keys):
            if key in v_bar_n_minus_one:
                v_bar[i] = v_bar_n_minus_one[key]
            else:
                v_bar[i] = 0
    ...
    if (state['e_k'] == 3):
        ...
        post_decision_state = [np.round(current_state + \
            x_su_k[i], decimals = -1) for i in valid_idx]
...
def sideSimulation(initial_state, x_hl_k, x_sl_k, x_su_k, v_bar_n, \
    ...
    while (not side_done) and (env.state['tau_k'] <= 400):
        ...
        valid_idx = apply_constraints(env.state, initial_state, x_hl_k, \
                                      x_sl_k, x_su_k)
...
def avi_agent(initial_state, seed, max_iterations, sim_at_iteration, \
    ...
    for n in range(N):
        ...
        for t in range(k - 1, 0, -1):
            ...
            if key in v_bar_n_minus_one:
                v_bar_n[key] = \
                        (1 - alpha_n) * v_bar_n_minus_one[key] + \
                        alpha_n * v_hat_t
            else:
                v_bar_n[key] = alpha_n * v_hat_t
        ...
        if (n % M == 0):
            ...
            print('Starting side simulations ...')

            # create a pool for multiprocessing
            pool = mp.Pool(mp.cpu_count())

            seeds = rng.integers(low = 1, high = 1e6, size = O)
            ...
            z = str(np.round(np.array([225, 225, 50, 0]), decimals = -1))
            tmp = pd.Series(data = {'eta' : eta, 'Iteration' : n, \
                'Value' : v_bar_n[z]})
...
if __name__ == '__main__':
    ...
    rempel_2024_initial_state = {
        ...
        'c_h' : 10,
        'c_s' : 50,
        'delta_h' : {'white' : 1, 'green' : 1, 'yellow' : 1, 'red' : 1},
        ...
        'tau_k' : 0,
        'e_k' : 0,
        'rho_e_k' : {'white' : 225, 'green' : 225, 'yellow' : 50, 'red' : 0},
